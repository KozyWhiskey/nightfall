...
### Must-ship school catalogue (IDs locked; flavor polish later)
...
| **Crafting / fusion / upgrading** | Craft nodes, Forge, Scriptorium | Fuel fuses, mods, power-ups (risk ladder applies) |
...
2. **Successful Return (or reclaim)** — when the scroll reaches Haven, it deposits into stock **and** unlocks **Haven knowledge** (Scriptorium / catalogue).
...
Percentages above are **starting defaults**, not final balance. Change the table; keep tier IDs stable.
...
Scriptorium building upgrades may **modify** these values (e.g. −brickChance on `risky`, cheaper soft cleanse) via named modifiers, not one-off code branches.
...
| Kind | Cleanse? |
|------|----------|
| **Soft curses** | **Yes** — can be healed/cleansed at Haven (Scriptorium or related building) for a resource sink. Not always free; may be incomplete on low building tier (domain-tunable). |
...
- [ ] First craft tutorial is Safe tier and teaches soft tradeoffs
