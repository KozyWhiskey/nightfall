...
export class HttpGameHost implements GameHost {
  ...
  public async getSnapshot(): Promise<GameSnapshot> {
    const response = await fetch("/api/snapshot", { headers: { accept: "application/json" }, cache: "no-store", credentials: "same-origin" });
  ...
  public async submit(command: CommandEnvelope): Promise<CommandResult> {
    const response = await fetch("/api/commands", { method: "POST", headers: { "content-type": "application/json", accept: "application/json" }, credentials: "same-origin", body: JSON.stringify(command) });
